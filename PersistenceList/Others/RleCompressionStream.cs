...
{
    public class RleCompressionStream<T> : Stream where T : RleImplementation<T>, new()
    {
        ...
        public override void Flush()
        {
            ...
            {
                byte[] buffer = Tools.ArrayCopy(m_writeBuffer, 0, m_writeBufferOffset);
                byte[] m_compressedBuffer = m_implementation.RleEncodeData(buffer);
                int sz = m_compressedBuffer.Length;
                //write header
                m_targetStream.Write(BitConverter.GetBytes(buffer.Length), 0, 4); //bytes decompressed
                m_targetStream.Write(BitConverter.GetBytes(sz), 0, 4); //bytes compressed
                //write data
        ...
        /// <param name="closeStream">The close stream.</param>
        public RleCompressionStream(Stream targetStream, int bufferSize, bool closeStream = false)
            : this (targetStream, new byte[bufferSize], closeStream)
        {
        }
        ...
        /// <param name="targetStream">The target.</param>
        /// <param name="closeStream">The close stream.</param>
        public RleCompressionStream(Stream targetStream, bool closeStream = false)
            : this(targetStream, 1 << 18, closeStream) // 256 kb
        {
        }
