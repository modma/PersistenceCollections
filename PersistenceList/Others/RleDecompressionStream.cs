...
{
    public class RleDecompressionStream<T> : Stream where T : RleImplementation<T>, new()
    {
        ...
        /// <summary>
        /// Method of buffer where compressed data is stored
        /// </summary>
        private readonly RleImplementation<T> m_implementation;
        ...
        /// <summary>
        /// Size of the compressed buffer returned in header.
        /// </summary>
        /// <param name="data">The buffer returned in header.</param>
        /// <returns></returns>
        private static int GetCompressedSize(byte[] data)
        {
        ...
        {
            int headerLength = 0;
            while (headerLength < HeaderSize)
            {
                int readed = m_targetStream.Read(m_header, headerLength, HeaderSize - headerLength);
                headerLength += readed;
                if (readed == 0) break;
            }
            ...
            int sizeCompressed = GetCompressedSize(m_header);
            if (m_readBuffer == null || m_readBuffer.Length < sizeCompressed)
            {
                m_readBuffer = new byte[sizeCompressed];
            }

            int bodyLength = 0;
            while (bodyLength < sizeCompressed)
            {
                int readed = m_targetStream.Read(m_readBuffer, bodyLength, sizeCompressed - bodyLength);
                bodyLength += readed;
                if (readed == 0) break;
            }
            ...
            // decompress
            m_unpackedLength = m_implementation.RleDecodeData(m_readBuffer, 0, null, ref m_unpackedBuffer, true);
            m_unpackedOffset = 0;
        }
