...
namespace QuickLZ
{
    ...
    public class QuickLZCompressionStream : Stream
    {
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            if (!_leaveOpen) _targetStream.Dispose();
        }
        ...
        /// <param name="leaveOpen">If True then the underlying stream is left open when this stream is closed, otherwise the underlying stream is closed when this stream is closed.</param>
        public QuickLZCompressionStream(Stream targetStream, byte[] writeBuffer, bool leaveOpen)
        {
        ...
        /// <param name="leaveOpen">If True then the underlying stream is left open when this stream is closed, otherwise the underlying stream is closed when this stream is closed.</param>
        public QuickLZCompressionStream(Stream targetStream, int bufferSize, bool leaveOpen = false)
            : this(targetStream, new byte[bufferSize], leaveOpen)
        {
        }
        ...
        /// <param name="targetStream">The target.</param>
        /// <param name="leaveOpen">If True then the underlying stream is left open when this stream is closed, otherwise the underlying stream is closed when this stream is closed.</param>
        public QuickLZCompressionStream(Stream targetStream, bool leaveOpen = false)
            : this(targetStream, 1 << 20, leaveOpen) // Yes, this is 1 MB
        {
        }
    }
}
